...
> **v0.2 方向**：✅ 枢纽化已立项（Git 分发 × 跨 Agent 引用 × 创作套件），详见 `docs/PLAN-06-枢纽化.md`，开发分支 `v0.2.0`；卡牌化提案暂放（缓办不废）
...
#### v0.2 方向拍板：枢纽化（PLAN-06）

- ✅ boss 拍板枢纽化为 v0.2 主线：Hub 引用 → 校验器 → 创作套件 → Git 分发（约 3-4 周）
- ✅ 卡牌化提案（PROPOSAL-v0.2-卡牌化.md）暂放，本周期不做趣味化
- ✅ v0.2 不内置 Python，规范校验器 Rust 原生实现（官方 skill-creator 规则已由 Paw 从原件提取，见 PLAN-06 §3.1）
- ⏸️ 使用统计范围待定，需进一步讨论（分层底稿见 PLAN-06 §2.3）
- ✅ `v0.2.0` 分支已从 main 切出（注意：工作区留有 v0.1 未提交的 Tip.tsx 修改）
